-- Admin Portal Schema for NVHealth Labs (PostgreSQL)
...
CREATE TABLE IF NOT EXISTS test_results_enhanced (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TRIGGER update_diagnostic_centers_admin_updated_at 
    BEFORE UPDATE ON diagnostic_centers_admin
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_lab_tests_admin_updated_at 
    BEFORE UPDATE ON lab_tests_admin
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_center_test_pricing_updated_at 
    BEFORE UPDATE ON center_test_pricing
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_test_packages_updated_at 
    BEFORE UPDATE ON test_packages
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_test_results_enhanced_updated_at 
    BEFORE UPDATE ON test_results_enhanced
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_diagnostic_centers_admin_status ON diagnostic_centers_admin(status);
CREATE INDEX IF NOT EXISTS idx_diagnostic_centers_admin_city ON diagnostic_centers_admin(city);
CREATE INDEX IF NOT EXISTS idx_lab_tests_admin_category ON lab_tests_admin(category);
CREATE INDEX IF NOT EXISTS idx_lab_tests_admin_status ON lab_tests_admin(status);
CREATE INDEX IF NOT EXISTS idx_bookings_enhanced_status ON bookings_enhanced(status);
CREATE INDEX IF NOT EXISTS idx_bookings_enhanced_date ON bookings_enhanced(appointment_date);
CREATE INDEX IF NOT EXISTS idx_test_results_enhanced_booking ON test_results_enhanced(booking_id);
